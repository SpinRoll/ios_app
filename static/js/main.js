document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleFormSubmit(e) {
        ...
        if (!validateForm()) {
            ...
            showToast('Please correct the errors in the form', 3000);
            return;
        }
        ...
        showToast('Generating QR Code...', 1000);
...
function initializeActionSheet() {
    ...
    // Add show action sheet trigger to share button
    const shareButton = document.querySelector('#shareCard');
    if (shareButton) {
        shareButton.addEventListener('click', showActionSheet);
    }

    buttons.forEach(button => {
        button.addEventListener('click', () => {
            hideActionSheet();
            if (button.classList.contains('destructive')) {
                hapticFeedback('heavy');
            }
        });
    });
...
function initializeContextMenu() {
    const contextMenu = document.querySelector('.ios-context-menu');
    const menuItems = document.querySelectorAll('.context-menu-item');

    function showContextMenu(e) {
        e.preventDefault();
        contextMenu.style.left = `${e.touches ? e.touches[0].clientX : e.clientX}px`;
        contextMenu.style.top = `${e.touches ? e.touches[0].clientY : e.clientY}px`;
...
function showToast(message, duration = 2000) {
    const toast = document.querySelector('.ios-toast');
